...
本 Demo 展示 UniRTOS 上基于 FTP 的 FOTA（空中固件升级）基础开发流程，适合作为远程固件下载与升级流程开发的入门样例。

- 演示在 FOTA 任务开始前激活 PDP 数据连接
- 演示初始化并配置 FTP 客户端上下文（PDP CID、超时、认证参数及传输选项）
- 演示通过 FTP 下载固件包并将数据包写入 FOTA 处理流程
- 包含结果状态处理、错误日志输出及基础的可重试控制流程
- 便于扩展为带认证 FTP 服务、多版本回滚策略与量产级升级编排
