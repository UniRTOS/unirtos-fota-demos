...
本 Demo 展示 UniRTOS 上基于 HTTP 的 FOTA（空中固件升级）基础开发流程，适合作为远程固件分发与在线升级开发的入门样例。

- 演示通过 HTTP GET 下载固件，并解析响应码与内容长度
- 支持定长传输与分块传输（chunked）模式，并处理首包初始化流程
- 演示将接收数据包写入 FOTA 模块并跟踪总体下载进度
- 包含文件大小容量校验、异常响应处理与提前退出保护
- 便于扩展为断点续传（Range）、重定向策略优化与企业级安全升级流程
